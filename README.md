...
- Mỗi bước của `xs128` đều có thể biểu diễn là một ma trận $A$ kích thước $128 \times 128$ (64 bit của `state0` và 64 bit của `state1`) trên $GF(2)$ sao cho $\text{state}_{t+1} = A * \text{state}_i$.
